...
/**
  * This is an event sourced entity. It has a state, [[ExternaltestingState]], which
  ...
  */
class ExternaltestingEntity extends PersistentEntity {

  override type Command = ExternaltestingCommand[_]
  override type Event = ExternaltestingEvent
  override type State = ExternaltestingState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: ExternaltestingState = ExternaltestingState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case ExternaltestingState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        ExternaltestingState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class ExternaltestingState(message: String, timestamp: String)

object ExternaltestingState {
  /**
    ...
    */
  implicit val format: Format[ExternaltestingState] = Json.format
}

/**
  * This interface defines all the events that the ExternaltestingEntity supports.
  */
sealed trait ExternaltestingEvent

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends ExternaltestingEvent
...
/**
  ...
  */
sealed trait ExternaltestingCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends ExternaltestingCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String, organization: Option[String]) extends ExternaltestingCommand[String]
...
/**
  ...
  * application loader.
  */
object ExternaltestingSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[ExternaltestingState]
  )
}
